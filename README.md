...
2. Copy the pre-built country folders (e.g. `ES/`, `FR/`, `GB/`, …) into `models/` — each contains `simu_data/` (demand & weather profiles) and `weather/` (renewable capacity factors) so you can run TD-ACOPF directly.
